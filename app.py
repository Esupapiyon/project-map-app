...
COMPATIBILITY_MAP = {
    0: ["己 (THE NURTURER)", "庚 (THE HERO)", "丁 (THE MUSE)"], 1: ["庚 (THE HERO)", "辛 (THE IDOL)", "丙 (THE PROTAGONIST)"],
    2: ["辛 (THE IDOL)", "壬 (THE NOMAD)", "戊 (THE ANCHOR)"], 3: ["壬 (THE NOMAD)", "癸 (THE COUNSELOR)", "己 (THE NURTURER)"],
    4: ["癸 (THE COUNSELOR)", "甲 (THE LEADER)", "庚 (THE HERO)"], 5: ["甲 (THE LEADER)", "乙 (THE CONNECTOR)", "辛 (THE IDOL)"],
    6: ["乙 (THE CONNECTOR)", "丙 (THE PROTAGONIST)", "壬 (THE NOMAD)"], 7: ["丙 (THE PROTAGONIST)", "丁 (THE MUSE)", "癸 (THE COUNSELOR)"],
    8: ["丁 (THE MUSE)", "戊 (THE ANCHOR)", "甲 (THE LEADER)"], 9: ["戊 (THE ANCHOR)", "己 (THE NURTURER)", "乙 (THE CONNECTOR)"]
}
...
def calculate_big5(answers):
    """
    TIPI-Jの回答(1-7)からビッグファイブスコア(0-12)を算出し、
    レーダーチャート用に1-5段階へ正規化する
    """
    # 回答は1-7の数値。キーはQ1, Q2...
    scores_raw = {
    ...
    # 1-5段階への正規化ロジック
    # TIPI-Jの範囲は 2〜14点。中央値は8。
    # 1(最小)〜5(最大)にマッピングする: 1 + (raw - 2) * 4 / 12
    scores_normalized = {}
...
if start_btn:
    ...
    # --- Dual Radar Chart (Fate vs Science) ---
    ...
    with col_r1:
        ...
        # 科学スコア (マッピング: 外向, 開放, 協調, 勤勉, 情緒安定[逆神経症])
        # Emotional Stability = 12 - Neuroticism (roughly mapped to 1-5 scale logic earlier)
        # Note: calculate_big5 function handles normalization 1-5.
        # Mapping: Identity->Extraversion, Create->Openness, Economy->Agreeableness, Status->Conscientiousness, Vitality->Stability
        
        # 情緒安定性は「神経症傾向」の逆なので、5点満点なら (6 - score) 的な反転が必要だが、
        # ここでは簡易的に calculate_big5 で出した Neuroticism をそのまま使うと「敏感さ」になる。
        # Vitality(知性)とNeuroticism(敏感さ)は必ずしも対ではないが、チャート比較用として配置。
        
        science_vals = [
            big5_norm['Extraversion'], 
            big5_norm['Openness'], 
            big5_norm['Agreeableness'], 
            big5_norm['Conscientiousness'], 
            6 - big5_norm['Neuroticism'] # 神経症傾向が低い＝安定性が高い(Vitality)として反転表示
        ]
        ...
        fig.add_trace(go.Scatterpolar(
            r=fate_vals, theta=categories, fill='toself', name='Fate (宿命)',
            line_color='#00c853', opacity=0.7
        ))
        
        # 2. Science (Blue) - TIPI-J
        fig.add_trace(go.Scatterpolar(
            r=science_vals, theta=categories, fill='toself', name='Personality (現在)',
            line_color='#2962ff', opacity=0.6
        ))
        ...
        st.caption("※緑: 生年月日から導かれた本来の資質 / 青: 心理テストによる現在の性格")
        st.markdown('</div>', unsafe_allow_html=True)
