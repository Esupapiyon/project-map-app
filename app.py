...
# ==========================================
# 1. Page Config & CSS (Ver Final_Fix_Html_Indent)
# ==========================================
st.set_page_config(
...
# 診断コンテンツ (Ver Final_Fix_Html_Indent)
DIAGNOSIS_CONTENT = {
    0: { # Type 1: 甲 (Wood+)
...
# ==========================================
# 5. UI Component Function (Ver Final_Physical_Fix)
# ==========================================
def render_result_component(content, fate_code, fate_scores, big5_norm=None, is_catalog=False, key_suffix=""):
    ...
    if not is_catalog:
        ...
        # メッセージ作成
        line_text_lines = [
            f"【閲覧希望】裏・ステータス診断の結果（Type{current_type_id}: {content['name']}）のレポートをください"
        ]

        # Big5スコアの追加
        if big5_norm:
            label_map = {
                "Extraversion": "外向性", "Openness": "開放性", 
                "Agreeableness": "協調性", "Conscientiousness": "勤勉性", 
                "Neuroticism": "安定性"
            }
            line_text_lines.append("\n▼ 現在の裏ステータス")
            for eng_key, jp_label in label_map.items():
                val = big5_norm.get(eng_key, 3.0)
                # 安定性(Neuroticism)の反転処理（チャートに合わせる）
                if eng_key == "Neuroticism":
                    val_display = round(6 - val, 1)
                else:
                    val_display = val
                line_text_lines.append(f"・{jp_label}: {val_display}")
