...
# ==========================================
# 1. Page Config & CSS (Ver Final_Fixed)
# ==========================================
st.set_page_config(
...
# ==========================================
# 5. UI Component Function (Ver Final_Fixed)
# ==========================================
def render_result_component(content, fate_code, fate_scores, big5_norm=None, is_catalog=False, key_suffix=""):
    ...
    # 長文解説の直前に見出し（削除指示済みだが、念のため序文として表示）
    ...
    # フック文章 (不要なst.success/st.info等は削除済み)
    # チャートエリア
    ...
    if not is_catalog:
        ...
        # チラ見せエリア (HTMLバグ修正済み)
        # style属性内の波括弧 {} は使用せず、styleタグで定義済みのクラスを使用するか、
        # f-stringで展開されないように注意する。ここではクラスベースで実装済み。
        st.markdown(f"""
...
# ==========================================
# 6. Main UI Application (Ver Final_Fixed)
# ==========================================
